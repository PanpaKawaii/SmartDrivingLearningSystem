...
export default function TrafficSignFlipBook() {
    ...
    return (
        <div className='traffic-sign-flip-book-container container'>
            <p>{currentPage}</p>
            <button className='btn' onClick={() => setCurrentPage(p => Math.max(p - 1, -0.5))}><i className='fa-solid fa-chevron-left' /></button>
            <FlipBook
                pages={pages}
            >
                {pages.map((p, i) => (
            ...
            <button className='btn' onClick={() => setCurrentPage(p => Math.min(p + 1, pages?.length + 0.5))}><i className='fa-solid fa-chevron-right' /></button>
        </div>
    )
}
